...
function run_hotspot {
        $JAVA_HOME/bin/java \
                -cp build/libs/factorization-1.0-all.jar \
                com.factorization.Factorization
}
...
function build_ni {
	...
	$JAVA_HOME/bin/native-image \
		--no-fallback \
		-cp libs/man-factors-1.0-all.jar:$ARGO_HOME/graalvisor-lib/build/libs/graalvisor-lib-1.0-guest.jar \
		...
		$NI_BIN_OPTS \
		-H:Name=libmanfactors
}

function build_ni_standalone {
	NI_BIN_OPTS="com.factorization.Factorization"
	build_ni
}
