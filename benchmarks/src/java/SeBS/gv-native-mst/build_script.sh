...
function build_ni {
	...
	$JAVA_HOME/bin/native-image \
			--no-fallback \
			-cp $CLASS_PATH:../libs/mst-1.0-all.jar:$ARGO_HOME/common/build/libs/faastion-lib-1.0-guest.jar \
...
function build_native_library {
	gcc $CFLAGS -Wl,--no-undefined -I"${IGRAPH_HOME}/include/igraph" -L"${IGRAPH_HOME}/lib" -o $CORE_DIR/shared/lib$BENCHMARK_NAME-jni.so $DIR/src/main/c/MST.c -ligraph -lm -fopenmp -lstdc++
}
