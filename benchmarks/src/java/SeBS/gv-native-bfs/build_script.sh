...
function build_native_binary {
	...
	export LD_LIBRARY_PATH=$CORE_DIR/shared:libs:$CURRENT_LIBRARY_PATH
...
function build_ni {
	...
	$JAVA_HOME/bin/native-image \
			--no-fallback \
			-cp $CLASS_PATH:../libs/bfs-1.0-all.jar:$ARGO_HOME/common/build/libs/faastion-lib-1.0-guest.jar \
...
function build_native_library {
	gcc $CFLAGS -Wl,--no-undefined -I"${IGRAPH_HOME}/include/igraph" -L"${IGRAPH_HOME}/lib" -o $CORE_DIR/shared/lib$BENCHMARK_NAME-jni.so $DIR/src/main/c/BFS.c -ligraph -lm -fopenmp -lstdc++	
}
