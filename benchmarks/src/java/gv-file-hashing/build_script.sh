...
function build_ni {
	...
	$JAVA_HOME/bin/native-image \
			--no-fallback \
			--enable-url-protocols=http \
			-cp $CLASS_PATH:libs/filehashing-1.0-all.jar:$ARGO_HOME/graalvisor-lib/build/libs/graalvisor-lib-1.0-guest.jar \
...
function build_native_library {
	gcc --shared -fpic $CFLAGS -o $GRAALVISOR_HOME/build/libs/lib$BENCHMARK_NAME-jni.so $DIR/src/main/c/FileHashing.c
}
...
function manipulate_bytecode {
	...
	ENTRYPOINT="com.jni.FileHashing"
	TOOL="JNITemplateBuilder"

	rm -f $GRAALVISOR_HOME/build/libs/lib${FUNCTION_ID}-hash.so
