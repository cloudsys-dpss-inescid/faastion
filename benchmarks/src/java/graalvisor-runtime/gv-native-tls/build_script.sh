...
function build_ni {
	...
	$JAVA_HOME/bin/native-image \
			--no-fallback \
			-cp $CLASS_PATH:../libs/tls-1.0-all.jar:$ARGO_HOME/graalvisor-lib/build/libs/graalvisor-lib-1.0-guest.jar \
...
function build_native_library {
	gcc $CFLAGS -O0 -fno-optimize-sibling-calls -Wl,--no-undefined -o $GRAALVISOR_HOME/shared/lib$BENCHMARK_NAME-jni.so.1.0 $DIR/src/main/c/TLS.c
	gcc $CFLAGS -O0 -fno-optimize-sibling-calls -Wl,--no-undefined -o $GRAALVISOR_HOME/shared/lib$BENCHMARK_NAME-jni.so.2.0 $DIR/src/main/c/TLS_2.c
	gcc $CFLAGS -O0 -fno-optimize-sibling-calls -Wl,--no-undefined -o $GRAALVISOR_HOME/shared/lib$BENCHMARK_NAME-jni.so.3.0 $DIR/src/main/c/TLS_3.c
}
...
function manipulate_bytecode {
	...
	$DEF_JAVA_HOME/bin/java -cp $CLASS_PATH $ENTRYPOINT build/libs/tls-1.0-all.jar
