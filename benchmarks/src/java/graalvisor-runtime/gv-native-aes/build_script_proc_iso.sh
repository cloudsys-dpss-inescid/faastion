...
function build_native_library {
        gcc $CFLAGS -o build/libs/lib$BENCHMARK_NAME-jni.so $DIR/src/main/c/AESEncryption.c -lssl -lcrypto
        mv build/libs/lib$BENCHMARK_NAME-jni.so $ARGO_HOME/graalvisor/build/libs
}
