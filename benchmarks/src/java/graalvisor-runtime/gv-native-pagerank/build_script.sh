...
function build_native_library {
	gcc $CFLAGS -Wl,--no-undefined -I"${IGRAPH_HOME}/include/igraph" -L"${IGRAPH_HOME}/lib" -o $GRAALVISOR_HOME/shared/lib$BENCHMARK_NAME-jni.so $DIR/src/main/c/PageRank.c -ligraph -lm -fopenmp -lstdc++
}
