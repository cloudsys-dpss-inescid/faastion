...
function build_native_library {
	gcc --shared -fpic $CFLAGS -o $GRAALVISOR_HOME/build/libs/lib$BENCHMARK_NAME-jni.so $DIR/src/main/c/MatrixMultiplication.c
}
...
function manipulate_bytecode {
	...
	rm -f $GRAALVISOR_HOME/build/libs/lib${FUNCTION_ID}-multiply.so
