...
function gv_httpequest {
	BENCHMARK_DIR="$BENCHMARKS_HOME/src/java/gv-httprequest"
}
...
function gv_matrixmul {
	BENCHMARK_DIR="$BENCHMARKS_HOME/src/java/gv-matrixmul"
}

function gv_factorization {
	BENCHMARK_DIR="$BENCHMARKS_HOME/src/java/gv-factorization"
}
...
native_benchmarks=(gv_native_hw gv_native_factors gv_filehashing gv_native_matmul)
managed_benchmarks=(gv_hello_world gv_aes_encryption gv_httprequest gv_matrixmul gv_factorization)

build_duration=$(echo "scale=2; ${#native_benchmarks[@]} * 40 / 60" | bc)
total_duration=$(echo "scale=2; ($CONCURRENCY_LEVEL + 2) * $build_duration" | bc)
total_duration=$(echo "scale=2; $total_duration + (${#managed_benchmarks[@]} * 40 * 2 / 60)" | bc)
