...
def create_subplot(benchmark, ax):
    ...
    for baseline in baselines:
        ...
        latencies = benchmarks[benchmark][baseline]["latency"]
        memories = benchmarks[benchmark][baseline]["memory"]
        if len(latencies) != len(memories):
            raise Exception(f'Invalid shapes: {benchmark} - {baseline}: ({len(latencies)},) != ({len(memories)},)')
        density = [latencies[i] / memories[i] for i in range(len(latencies))]
        ax.plot(x, density, label=baseline, color=COLORS[baseline], marker=MARKERS[baseline])
...
def create_plot():
    ...
    ax_count = 0
    axes[ax_count].set_ylabel("Density (Req/sec/MiB)")
    for benchmark in BENCHMARKS:
        create_subplot(benchmark, axes[ax_count])
        ax_count += 1

    handles, labels = axes[0].get_legend_handles_labels()
    fig.legend(handles, labels, loc="lower center", ncol=len(REQUESTS), bbox_to_anchor=(0.5, -0.02), borderaxespad=0.)
    fig.suptitle("Density per Benchmark")
    plt.tight_layout()
