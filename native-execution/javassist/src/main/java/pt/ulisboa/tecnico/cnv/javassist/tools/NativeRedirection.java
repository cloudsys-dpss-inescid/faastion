...
public class NativeRedirection extends CodeDumper {
	...
	public static void createHeader(String[] jniTypes, String returnJniType, String className, String gateName)
			throws IOException {
		...
		if (file.exists()) {
			...
			lines.add(
					"\t(JNIEnv *, jclass" + (jniTypes.length > 0 ? ", " : "") + String.join(", ", jniTypes) + ");\n\n");
	...
	public static void createSnippet(String[] jniTypes, String returnJniType, String methodName, String className,
		...
		try (FileWriter writer = new FileWriter(file)) {
			 ...
			 * writer.write("static __thread " + returnJniType +
			 * " (JNICALL *native_method)(JNIEnv *env, jobject obj" + typeArgs +
			 * ") = NULL;\n");
			 */
			 ...
			 * + "\");\n");
			 * writer.write("\t\tif (native_method == NULL) {\n");
			 * writer.write("\t\t\tfprintf(stdout, \"Failed to find the symbol: " +
			 * nativeMethodName + "\\n\");\n");
			 * writer.write("\t\t\texit(EXIT_FAILURE);\n");
			 * writer.write("\t\t}\n");
			 * writer.write("\t}\n\n");
			 */
			writer.write("\tvoid (*native_method)(JNIEnv*, jobject" + (jniTypes.length > 0 ? ", " : "")
					+ String.join(", ", jniTypes) + ") = dlsym(RTLD_DEFAULT, \"" + nativeMethodName + "\");\n");
			writer.write("\tif (native_method == NULL) {\n");
			...
			writer.write("\t}\n");
			writer.write("\tvoid (*native_method)(JNIEnv*, jobject" + (jniTypes.length > 0 ? ", " : "")
					+ String.join(", ", jniTypes) + ") = dlsym(open_lib, \"" + nativeMethodName + "\");\n");
			writer.write("\tif (native_method == NULL) {\n");
			writer.write("\t\t\tfprintf(stdout, \"Failed to find the symbol: " + nativeMethodName + "\\n\");\n");
			writer.write("\t\t\texit(EXIT_FAILURE);\n");
			writer.write("\t\t}\n\n");
	...
	public static void declareCallGate(CtClass clazz, String[] parameters, CtClass returnType, String gateName)
