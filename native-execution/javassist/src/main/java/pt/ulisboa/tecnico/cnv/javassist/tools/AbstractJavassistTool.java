...
public abstract class AbstractJavassistTool implements ClassFileTransformer {
    ...
    protected void transform(CtClass clazz) throws Exception {
        ...
        Iterator<String> iterator = refClasses.iterator();
        
        while (iterator.hasNext()) {
            String className = iterator.next();
            CtClass ct = classPool.get(className);

            Collection<String> damas = ct.getRefClasses();
            Iterator<String> damiterator = damas.iterator();
        
            while (damiterator.hasNext()) {
                System.out.println(ct.getName() + " references " + damiterator.next());
            }

            for (CtBehavior behavior : ct.getDeclaredBehaviors()) {
                boolean isNative = Modifier.isNative(behavior.getModifiers());
                if (isNative)
                    System.out.println(behavior.getLongName() + " is Native.");
            }
        }
